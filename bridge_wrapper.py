...
class YOLOv7_DeepSORT:
    ...
    def track_video_stream(self, frame: str, frame_num: int = 0,
                           count_objects: bool = False, verbose: int = 0):
        """
        Tracking the frames feeding into the tracker
        frame: frame of a video in bytes
        frame_num: the order of the frame, for display only
        count_objects: count objects being tracked on screen
        verbose: print details on the screen allowed values 0,1,2
        :return:
        result: annotated image displaying detected results and tracking path
        detections: Detect object in list of Detection object
        """
        if verbose >= 1: start_time = time.time()
